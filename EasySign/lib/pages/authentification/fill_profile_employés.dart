...
class FillProfileEmployes extends StatefulWidget {
  final String uid;
  const FillProfileEmployes({Key? key, required this.uid}) : super(key: key);

  @override
  _FillProfileEmployesState createState() => _FillProfileEmployesState();
}

class _FillProfileEmployesState extends State<FillProfileEmployes> {
  ...
  Future uploadImageToFirebase(File img, String url) async {
    ...
    taskSnapshot.ref.getDownloadURL().then((value) {
      firestoreInstance.collection('users').doc(widget.uid).update({'pdp': value});
    });
  }
  ...
  Future _uploadFile() async {
    final path = 'users/' + widget.uid + '/cni';
    ...
    TaskSnapshot taskSnapshot = await uploadTask.whenComplete(() => null);
    taskSnapshot.ref.getDownloadURL().then((value) {
      firestoreInstance.collection('users').doc(widget.uid).update({'cni': value});
    });
  }
                ...
                child: Container(
                  ...
                  child: Align(
                    ...
                    child: Padding(
                      ...
                      child: Text(
                        pickedFile == null ? "Selectionnez une carte d'identité" : pickedFile!.name,
                        style: GoogleFonts.montserrat(
                          textStyle: TextStyle(
                            fontWeight: FontWeight.w400,
                ...
                child: UI_gradient_button(
                    ...
                    onPressed: () {
                      if (!nameController.text.contains(new RegExp(r'[A-Z, a-z]'))) {
                        setState(() {
                          errorMessage = "Merci d'entrer nom et prénom.";
                        });
                      } else if (pickedFile == null) {
                        setState(() {
                          errorMessage = "Merci de séléctionner une carte d'identité.";
                        });
                      } else {
                        if (pic) {
                          uploadImageToFirebase(
                            add_photo,
                            'users/' + widget.uid + '/pdp',
                          );
                        }
                        _uploadFile();
                        List searchKeywords = [];
                        ...
                        for (int i = 0; i < nameController.text.length; i++) {
                          if (searchKeywords.isEmpty) {
                            searchKeywords.add(nameController.text[i].toLowerCase());
                          } else {
                            searchKeywords.add(searchKeywords[searchKeywords.length - 1] + nameController.text[i].toLowerCase());
                          }
                        }
